...
public class ScheduleQuery  implements ApplicationRunner{
    ...
    public void doScheduleQuery() {
        List<BaseInfo> baseInfos = ScheduleQueryHelper.getAvailableBases().stream().filter(base -> base.getDevType().equals("0020012") /**&& base.getDevNo().equals("22")*/).collect(Collectors.toList());
//          List<BaseInfo> baseInfos = ScheduleQueryHelper.getAvailableBases().stream().filter(base -> base.getDevType().equals("0020005")||base.getDevType().equals("0020006")).collect(Collectors.toList());
